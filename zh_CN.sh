...
if [ "${release}" == "centos" ]; then
	...
	cd /etc
	rm -rf locale.conf
	wget https://raw.githubusercontent.com/VPN-V2Ray/zh_CN/main/locale.conf > /dev/null 2>&1
	...
	cd /etc/
	wget https://raw.githubusercontent.com/VPN-V2Ray/zh_CN/main/locale.gen > /dev/null 2>&1
	locale-gen
	cd /etc/default/
	wget https://raw.githubusercontent.com/VPN-V2Ray/zh_CN/main/locale.conf > /dev/null 2>&1
	cp locale.conf locale
...
clear
echo ""
echo "Your VPS Language setting is changed to Chinese"
echo "Restart your VPS and reconnect to VPS CLI to check it"
echo ""
